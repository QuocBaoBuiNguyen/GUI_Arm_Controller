...
class CameraController(object):
    def __init__(self,  f_cb, f2_cb):
        self.thread = {}
        self.photo_capture_callback = f_cb
        self.chosePosDependOnLabelcb = f2_cb
    ...
    def start_capture_video(self):
        self.thread[1] = live_stream(self.chosePosDependOnLabelcb, index=1)
        self.thread[1].start()
...
class live_stream(QThread):
    ...
    def __init__(self, f_cb, index):
        ...
        self.chosePosDependOnLabelcblt = f_cb
    ...
    def load_model(self):
        ...
        model = torch.hub.load('ultralytics/yolov5', 'custom',
                               path='D:/NCKH/AppQt/Project/ArmGUI/model/YOLOv5_DrinksLogos_V4')
        return model
    ...
    def plot_boxes(self, results, frame):
        ...
        for i in range(n):
            ...
            if row[4] >= 0.9:
                ...
                cv2.putText(frame, str(center[0]) + " " + str(center[1]), center, cv2.FONT_HERSHEY_SIMPLEX, 0.9, bgr, 2)
    ...
    def run_program(self):
        ...
        while True:
            ...
            print(objectLabel, objectLabel != "No object")
            # if objectLabel != "No object":
            if (100 <= center[0] <= 300) and (50 <= center[1] <= 300) and (objectLabel != "No object"):
                print("co goi if")
                self.chosePosDependOnLabelcblt(objectLabel)
            if not self.gg:
